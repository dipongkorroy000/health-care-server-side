...
const insertIntoDB = async (payload: ISchedule) => {
  ...
  while (currentDate <= lastDate) {
    ...
    const endDateTime = new Date(addMinutes(addHours(format(lastDate, "yyyy-MM-dd"), Number(endTime.split(":")[0])), Number(endTime.split(":")[1])));
...
const getAllFromDB = async (user: IJWTPayload, filters: IFilterRequest, options: IPaginationOptions) => {
  ...
  const doctorSchedules = await prisma.doctorSchedules.findMany({where: {doctor: {email: user.email}}, select: {scheduleId: true}});
  ...
  const total = await prisma.schedule.count({where: {...whereConditions, id: {notIn: doctorScheduleIds}}});
...
export const ScheduleService = {insertIntoDB, getAllFromDB, deleteScheduleFromDB, getByIdFromDB};

...
const createAppointmentWithPayLater = catchAsync(async (req: Request & {user?: IJWTPayload}, res: Response) => {
  const result = await AppointmentService.createAppointmentWithPayLater(req.user as IJWTPayload, req.body);
...
const initiatePayment = catchAsync(async (req: Request & {user?: IJWTPayload}, res: Response) => {
  const result = await AppointmentService.initiatePaymentForAppointment(req.user as IJWTPayload, req.params.id as string);

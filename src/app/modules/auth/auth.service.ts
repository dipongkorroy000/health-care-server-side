...
const login = async (payload: Login) => {
  ...
  const accessToken = await jwtHelper.generateToken({
    ...
    expireIn: config.jwt.expires_in,
  });
  ...
  const refreshToken = await jwtHelper.generateToken({
    ...
    expireIn: config.jwt.refresh_token_expires_in,
  });
...
const refreshToken = async (token: string) => {
  ...
  const userData = await prisma.user.findUniqueOrThrow({where: {email: decodedData.email, status: UserStatus.ACTIVE}});

  const accessToken = jwtHelper.generateToken({email: userData.email, role: userData.role, secret: config.jwt.jwt_secret, expireIn: config.jwt.expires_in});
...
const changePassword = async (user: any, payload: any) => {
  const userData = await prisma.user.findUniqueOrThrow({where: {email: user.email, status: UserStatus.ACTIVE}});
  ...
  await prisma.user.update({where: {email: userData.email}, data: {password: hashedPassword, needPasswordChange: false}});

  return {message: "Password changed successfully!"};
};

const forgotPassword = async (payload: {email: string}) => {
  const userData = await prisma.user.findUniqueOrThrow({where: {email: payload.email, status: UserStatus.ACTIVE}});

  const resetPassToken = jwtHelper.generateToken({
    email: userData.email,
    role: userData.role,
    secret: config.jwt.reset_pass_secret,
    expireIn: config.jwt.reset_pass_token_expires_in,
  });
...
const resetPassword = async (token: string | null, payload: {email?: string; password: string}, user?: {email: string}) => {
  ...
  if (token) {
    ...
    if (!decodedToken) throw new ApiError(httpStatus.FORBIDDEN, "Invalid or expired reset token!");

    // Verify email from token matches the email in payload
    if (payload.email && decodedToken.email !== payload.email) throw new ApiError(httpStatus.FORBIDDEN, "Email mismatch! Invalid reset request.");
  ...
  // Case 2: Authenticated user with needPasswordChange (newly created admin/doctor)
    ...
    const authenticatedUser = await prisma.user.findUniqueOrThrow({where: {email: user.email, status: UserStatus.ACTIVE}});
  ...
  await prisma.user.update({where: {email: userEmail}, data: {password, needPasswordChange: false}});
};
...
export const AuthService = {login, refreshToken, changePassword, resetPassword, getMe, forgotPassword};

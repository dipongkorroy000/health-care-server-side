...
const insertIntoDB = async (user: IJWTPayload, payload: any) => {
  ...
  const appointmentData = await prisma.appointment.findUniqueOrThrow({ where: { id: payload.appointmentId } });
  ...
  return await prisma.$transaction(async (tnx) => {
    ...
    await tnx.doctor.update({ where: { id: appointmentData.doctorId }, data: { averageRating: avgRating._avg.rating as number } });
...
const getAllFromDB = async (filters: any, options: IPaginationOptions) => {
  ...
  if (doctorEmail) andConditions.push({ doctor: { email: doctorEmail } });

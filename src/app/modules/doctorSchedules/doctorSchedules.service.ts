...
const getMySchedule = async (filters: any, options: IPaginationOptions, user: IJWTPayload) => {
  ...
  if (Object.keys(filterData).length > 0) {
    if (typeof filterData.isBooked === "string" && filterData.isBooked === "true") filterData.isBooked = true;
...
const deleteFromDB = async (user: IJWTPayload, scheduleId: string) => {
  ...
  if (isBookedSchedule) throw new ApiError(status.BAD_REQUEST, "You can not delete the schedule because of the schedule is already booked!");
...
const getAllFromDB = async (filters: any, options: IPaginationOptions) => {
  ...
  if (searchTerm) andConditions.push({doctor: {name: {contains: searchTerm, mode: "insensitive"}}});

  if (Object.keys(filterData).length > 0) {
    if (typeof filterData.isBooked === "string" && filterData.isBooked === "true") filterData.isBooked = true;
    else if (typeof filterData.isBooked === "string" && filterData.isBooked === "false") filterData.isBooked = false;

    andConditions.push({AND: Object.keys(filterData).map((key) => ({[key]: {equals: (filterData as any)[key]}}))});
  }
...
export const DoctorScheduleService = {insertIntoDB, getMySchedule, deleteFromDB, getAllFromDB};

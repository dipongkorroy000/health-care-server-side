...
const getByIdFromDB = async (id: string): Promise<Patient | null> => {
  return await prisma.patient.findUnique({ where: { id, isDeleted: false } });
};
...
const softDelete = async (id: string): Promise<Patient | null> => {
  return await prisma.$transaction(async (transactionClient) => {
    ...
    await transactionClient.user.update({ where: { email: deletedPatient.email }, data: { status: UserStatus.DELETED } });
...
const updateIntoDB = async (user: IJWTPayload, payload: any) => {
  ...
  const patientInfo = await prisma.patient.findUniqueOrThrow({ where: { email: user.email, isDeleted: false } });
  ...
  return await prisma.$transaction(async (tnx) => {
    ...
    if (medicalReport) await tnx.medicalReport.create({ data: { ...medicalReport, patientId: patientInfo.id } });
...
export const PatientService = { getAllFromDB, getByIdFromDB, softDelete, updateIntoDB };
